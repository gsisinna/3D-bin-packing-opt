...
class Painter:
    ...
    def __init__(self, bins):
        ...
        # Assign a unique color to each item if not already assigned
        default_colors = plt.cm.get_cmap("tab20", len(
            self.items))  # Up to 20 distinct colors
        for idx, item in enumerate(self.items):
            if not hasattr(item, 'color') or not item.color:
                rgba = default_colors(idx)
                item.color = tuple(int(255 * c)
                                   for c in rgba[:3])  # Convert to RGB

    def _plotCube(self, plotter, x, y, z, dx, dy, dz, color='red', edge_color='black', text="", fontsize=15, opacity=0.5, edge_width=1.0):
        center = (x + dx / 2, y + dy / 2, z + dz / 2)
        cube = pv.Cube(center=center, x_length=dx, y_length=dy, z_length=dz)
        plotter.add_mesh(
            cube,
            color=color,
            opacity=opacity,
    ...
    def _plotCylinder(self, plotter, x, y, z, dx, dy, dz, color='blue', edge_color='black', text="", fontsize=12, opacity=0.5, edge_width=1.0):
        center = (x + dx / 2, y + dy / 2, z + dz / 2)
        radius = dx / 2
        cylinder = pv.Cylinder(center=center, direction=(
            0, 0, 1), radius=radius, height=dz)
        plotter.add_mesh(
            cylinder,
            color=color,
            opacity=opacity,
    ...
    def plotBoxAndItems(self, title="", alpha=0.4, write_num=False, fontsize=10):
        plotter = pv.Plotter()

        # Plot the bin container
        self._plotCube(
            plotter,
            0, 0, 0,
